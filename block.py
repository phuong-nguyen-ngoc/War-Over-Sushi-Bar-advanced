	...
	def update(self):
		...
		if self.health == 0:
			...
			if self.bonus==2:
				...
				self.image.set_colorkey(BLACK)
			if self.bonus==3:
				self.image = pg.image.load('recipe.png').convert()#speed boost
				self.image.set_colorkey(BLACK)
			if self.bonus ==4:   
				self.image = pg.image.load('boot-collect.png').convert()#speed boost
				self.image.set_colorkey(BLACK)
			if self.bonus == 5:
				self.image = pg.image.load('superbullet.png').convert()#speed boost
				self.image.set_colorkey(BLACK)
			if self.bonus == 6:
				self.image = pg.image.load('lamp.png').convert()#speed boost
				self.image.set_colorkey(BLACK)
